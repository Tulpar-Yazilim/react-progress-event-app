...
function ProgressUpdatePage() {
  ...
  const [ganttChartCreatedAt, setGanttChartCreatedAt] = React.useState(null);
  ...
  const saveProgress = async () => {
    return new Promise((resolve) => {
      ProgressEventDataService.update(params.id, {
        ProgressTitle: ganttChartTitle,
        CreatedAt: ganttChartCreatedAt,
  ...
  const getProgressEvents = (itemKey) => {
    ProgressEventDataService.get(itemKey).once("value", (item) => {
      const value = item.val();
      setGanttChartTitle(value.ProgressTitle);
      setGanttChartCreatedAt(value.CreatedAt);
      if (value.Tasks) {
    ...
    <>
      <Container fluid className="main-content-container px-4 pb-4">
        ...
        <Row noGutters className="page-header py-4">
          <Col md="10">
            <PageTitle
              title="Görev Çizelgesini Güncelle"
